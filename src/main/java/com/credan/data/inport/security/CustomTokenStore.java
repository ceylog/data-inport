...
import com.credan.data.inport.dao.OauthtokenMapper;
import com.credan.data.inport.entity.Oauthtoken;
...
public class CustomTokenStore implements TokenStore {
    ...
    @Autowired
    private OauthtokenMapper oauthtokenRepository;
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        oauthtoken.setAccesstoken(token.getValue().getBytes());
        oauthtoken.setRefreshtoken(token.getRefreshToken() == null ? null : token.getRefreshToken().getValue().getBytes());
        ...
        oauthtokenRepository.insert(oauthtoken);
    ...
    public OAuth2RefreshToken readRefreshToken(String tokenValue) {
        ...
        Oauthtoken oauthtoken = findByRefreshid(DigestUtils.md5Hex(tokenValue));
        if (oauthtoken.getRefreshed() == Boolean.TRUE) {
            throw new BadCredentialsException("RefreshToken Is Refreshed.");
        }
        OAuth2RefreshToken oAuth2RefreshToken = new DefaultOAuth2RefreshToken(oauthtoken.getRefreshtoken().toString());
    ...
    public OAuth2Authentication readAuthenticationForRefreshToken(OAuth2RefreshToken token) {
        ...
        Oauthtoken oauthtoken = findByRefreshid(DigestUtils.md5Hex(token.getValue()));
        try {
    ...
    public void removeAccessTokenUsingRefreshToken(OAuth2RefreshToken refreshToken) {
        ...
        Oauthtoken oauthtoken = findByRefreshid(DigestUtils.md5Hex(refreshToken.getValue()));
        oauthtoken.setRefreshed(Boolean.TRUE);
        oauthtokenRepository.updateByPrimaryKeySelective(oauthtoken);
    ...
    private Oauthtoken findByRefreshid(String refreshid){
        Oauthtoken o = new Oauthtoken();
        o.setRefreshid(refreshid);
        return oauthtokenRepository.selectOne(o);
    }
}